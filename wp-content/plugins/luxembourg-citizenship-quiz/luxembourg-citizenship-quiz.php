...
function lcq_enqueue_quiz_scripts() {
    ...
    if ( is_a( $post, 'WP_Post' ) && has_shortcode( $post->post_content, 'luxembourg_eligibility_quiz' ) ) {
        wp_enqueue_style( 'lcq-quiz-styles', plugin_dir_url( __FILE__ ) . 'lcq-styles.css', array(), '1.4' );
        wp_enqueue_script( 'lcq-quiz-script', plugin_dir_url( __FILE__ ) . 'lcq-quiz.js', array(), '1.4', true );
